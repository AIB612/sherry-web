    ...
    >
      ...
      >
        {item.videoUrl ? (
          ...
          <div
            className={`w-full h-full bg-gradient-to-br ${item.thumbnailBg} flex flex-col items-start justify-end p-8`}
          >
            ...
            {item.description && (
              <ul className="space-y-2 max-w-xl">
                {item.description.map((point, i) => (
                  <li
                    key={i}
                    className="text-white/70 text-[13px] leading-relaxed flex gap-2"
                  >
...
function TrackCard({ item, index }: { item: TrackItem; index: number }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`relative overflow-hidden w-full h-full`}>
          {item.image ? (
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-br ${item.thumbnailBg} transition-transform duration-[800ms] ease-[0.25,1,0.5,1] group-hover:scale-105`}
            />
          )}
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-6 lg:p-8 z-10">
            <div className="translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-[500ms] ease-[0.33,1,0.68,1]">
              <div className="flex items-center gap-2 mb-2">
                <p className="text-[9px] tracking-[0.2em] text-white/70 font-mono">
                  {item.year}
                </p>
                <span className="h-px w-4 bg-white/30" />
                <p className="text-[9px] tracking-[0.2em] text-white/50">
                  Switzerland
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {item.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="text-[8px] tracking-[0.15em] text-white/60 border border-white/20 px-2 py-1 backdrop-blur-sm"
                  >
                    {tag}
                  </span>
                ))}
      ...
      <AnimatePresence>
        {modalOpen && (
          <VideoModal item={item} onClose={() => setModalOpen(false)} />
        )}
      </AnimatePresence>
    </>
  );
  ...
  return (
    <section className="bg-white px-5 md:px-8 lg:px-20 py-16 md:py-28">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10 md:mb-12">
        <div>
          <p className="text-[10px] tracking-[0.35em] text-neutral-500 mb-4 font-medium">
            SELECTED WORK
          </p>
        ...
        <p className="text-neutral-600 text-sm max-w-xs leading-relaxed self-start">
          A decade of product design, AI engineering, and digital transformation
        ...
        <div className="mt-16 flex items-center justify-center">
          ...
          >
            <span className="relative z-10 flex items-center gap-3">
              {expanded
                ? "SHOW LESS"
                : `VIEW ALL ${trackItems.length} PROJECTS`}
              <svg
