...
export default function WorkDetailPage() {
  ...
  // 获取相邻项目
  const currentIndex = portfolioItems.findIndex(p => p.id === params.id);
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            {item.tags.map((tag) => (
              <span
                key={tag}
                className="text-sm border border-neutral-200 rounded-full px-4 py-2 hover:border-black transition-colors"
              >
                {tag}
              </span>
            ))}
